...
function generateIndexJS(cfg) {
  cfg.dirs.forEach( d => {
    var str    = `// this file is generated by the build process ${config.buildDate}\n\n`;
    var isArr  = Array.isArray(d);
    var dir    = isArr ? d[0] : d;
    var opts   = Object.assign( {}, { camelize: false, skip: [] }, isArr ? d[1] : {} );
    var camel  = opts.camelize ? camelize : function(n) { return n; };
    var iname  = `${dir}/index.js`;

    var exportNames = [];

    del.sync(iname);
    var files = glob.sync(`${dir}/*`);
    var skip = opts.skip.concat(cfg.skip || []);
    files.forEach( f => {
...
function task_make_indecies() {
  ...
  var dirs = config.apps
    ...
    .concat( [ 
      ...
      ['./app/mixins', { camelize: true } ]
    ]);
  generateIndexJS( { dirs, skip } );  
}
...
gulp.task('server-js',   ['server-clean'], () => { 
    task_make_indecies();
