...
class UserSearch extends QueryFilters(Query) {
  ...
  lookUpUsers( str, queryParams = {}, batchName = null ) {
    var q = mergeParams( { lookup: str }, this._defaultParams, queryParams );
    return this.searchUsers( q, batchName );
  }
  
  // search the entire users record
  searchUsers(params,batchName,deferThis) {
    ...
    return (deferThis || this).query(params,batchName)
                  .then( serialize( ccmixter.User.UserBasic ) );
  }

  findUser(id,batchName,deferThis) {
    var qparams = {
      user: id,
      dataview: 'user_basic',
      noarray: 1
    };
    return (deferThis || this).query(qparams,batchName).then( serialize( ccmixter.User.User ) );
  }

  // ids is a comma separated string of id's
  findUsers(ids,batchName) {
    var qp = { ids, t: 'user_list' };
    return this.query(qp,batchName)
